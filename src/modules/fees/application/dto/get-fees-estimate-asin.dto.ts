...
import {
  VALID_CURRENCIES,
  MARKETPLACE_IDS,
} from '../../../../shared/infrastructure/http/spapi/spapi.config';
...
export class GetFeesEstimateAsinDto {
  ...
  @ApiProperty({
    ...
    description:
      'ID del marketplace de Amazon (MX: A1AM78C64UM0Y8, US: ATVPDKIKX0DER, etc.)',
    minLength: 10,
    maxLength: 20,
  })
  @IsString()
  @Length(10, 20, {
    message: 'marketplaceId debe tener entre 10 y 20 caracteres',
  })
  ...
  @IsNumber(
    { maxDecimalPlaces: 2 },
    { message: 'listingPriceAmount debe ser un número con máximo 2 decimales' },
  )
  ...
  @IsIn(VALID_CURRENCIES, {
    message: `listingPriceCurrency debe ser una de: ${VALID_CURRENCIES.join(', ')}`,
  })
  ...
  @IsNumber(
    { maxDecimalPlaces: 2 },
    { message: 'shippingAmount debe ser un número con máximo 2 decimales' },
  )
  ...
  @ApiProperty({
    example: 'MXN',
    description:
      'Código de moneda ISO 4217 para el envío (debe coincidir con listingPriceCurrency)',
    required: false,
    enum: VALID_CURRENCIES,
  })
  @IsOptional()
  @IsString()
  @IsIn(VALID_CURRENCIES, {
    message: `shippingCurrency debe ser una de: ${VALID_CURRENCIES.join(', ')}`,
  })
