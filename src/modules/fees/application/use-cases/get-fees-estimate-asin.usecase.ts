...
export class GetFeesEstimateForAsinUseCase {
  ...
  async execute(dto: GetFeesEstimateAsinDto): Promise<FeesEstimateResponseDto> {
    ...
    try {
      ...
      const spapiResponse =
        await this.productFeesGateway.getMyFeesEstimateForASIN({
          asin: dto.asin,
  ...
  private validateBusinessRules(dto: GetFeesEstimateAsinDto): void {
    ...
    if (
      dto.shippingCurrency &&
      dto.shippingCurrency !== dto.listingPriceCurrency
    ) {
      throw new BadRequestException(
    ...
    if (dto.listingPriceAmount <= 0) {
      throw new BadRequestException('El precio de listado debe ser mayor a 0.');
    }
