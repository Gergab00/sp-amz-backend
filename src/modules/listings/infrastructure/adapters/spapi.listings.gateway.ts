...
export class SpapiListingsGateway implements ListingsApiPort {
  ...
  async getListingsItem(params: {
    ...
    try {
      this.logger.debug(
        `Llamando a SP-API getListingsItem: ${JSON.stringify(params)}`,
      );
      return await this.spapiClient.callAPI({
        ...
        query: {
          ...
          includedData: ['fulfillmentAvailability', 'offers', 'attributes'],
        },
      });
    } catch (error: any) {
      ...
      if (error?.code === 403)
        throw new Error('403: Permiso denegado en SP-API');
